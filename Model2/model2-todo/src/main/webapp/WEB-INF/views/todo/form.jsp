	...
	<div class="row mb-3">
		<div class="col">
			<div class="card">
				...
				<div class="card-body">
					<p class="card-text text-muted">일정정보를 입력하고 등록버튼을 클릭하세요</p>
					<form method="post" action="insert.hta"  novalidate>
						<div class="mb-3">
							<label class="form-label" for="category-field">카테고리</label> 
							<%-- 
								TodoFormController가 요청객체에 속성으로 저장한 전체 카테고리 목록정보를 조회해서
								<c:forEach var="변수명" items="${배열 혹은 List객체를 조회하는 표현식}"
										반복해서 출력할 html컨텐츠
								</c:forEach>
							 --%>
							<select class="form-select" id="category-field" name="categoryNo">
								...
								<%-- 
									1. 요청객체에 "categoryList"이름으로 저장된 속성을 조회하는 List<Category>객체가 조회된다.
									2. <c:forEach /> 태그를 사용해서 조회된 카테고리정보(List<Category>)로 <option>태그를 생성한다.
								
									<c:forEach var="변수명" items="${요청객체에 저장된 속성을 조회하는 표현식}"
										반복해서 출력할 html컨텐츠
									</c:forEach>
								--%>
