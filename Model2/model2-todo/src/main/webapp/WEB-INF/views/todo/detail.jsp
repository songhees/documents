	...
	<div class="row mb-3">
		<div class="col">
			<div class="card">
				...
				<div class="card-body">
					<form method="post" action="update.hta?no=${todoDto.no}&page=${param.page }"  novalidate>
						<div class="mb-3">
						...
						<div class="mb-3">
							...
							<input type="text" class="form-control" id="title-field" name="title" value='<c:out value="${todoDto.title }" />' />
						</div>
						<div class="mb-3">
							<label class="form-label" for="dueDate-field">처리예정일</label> 
							<input type="date" class="form-control" id="dueDate-field" name="dueDate" value='<fmt:formatDate value="${todoDto.dueDate }" pattern="yyyy-MM-dd" />' />
						</div><div class="mb-3">
							<label class="form-label" for="status-field">처리 상태</label> 
							<select class="form-select" id="status-field" name="statusNo">
								<c:forEach var="status" items="${statusList }">
									<option value="${status.no }" ${todoDto.statusNo eq status.no ? 'selected' : '' }>${status.title }</option>
								</c:forEach>
							</select>
						</div>
						<div class="mb-3">
							<label class="form-label" for="dueDate-field">내용</label> 
							<textarea class="form-control" rows="7" id="content-field" name="content"><c:out value="${todoDto.content }"/></textarea>
						</div>
						<div class="text-end ">
							<c:if test="${loginUser.no eq todoDto.userNo}">
