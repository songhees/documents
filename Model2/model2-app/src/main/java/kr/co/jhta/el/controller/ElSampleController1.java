...
public class ElSampleController1 implements Controller {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// HttpSession객체 획득하기
		// 클라이언트가 최초접속시 생성되고, 로그아웃하면 사라지는 객체
		HttpSession session = request.getSession();
		// ServletContext객체 획득하기
		// 웹서버가 켜질때 생성되고, 웹서버가 종료될 때 사라지는 객체
		ServletContext application = request.getServletContext();
		
		// 기본 자료형 타입의 값을 속성으로 저장하고, JSP에서 EL로 표현하기
		session.setAttribute("message", "EL연습하기 페이지 방문을 환영합니다.");
		session.setAttribute("viewCount", 1000);
		session.setAttribute("userDeleted", true);
		session.setAttribute("discountRate", 0.01);
		
		// String타입의 값을 속성으로 저장하고 JSP에서 EL로 표현하기
		application.setAttribute("deptName", "경영지원팀");
		...
		User user3 = new User("lee", "zxcv3333", "이순신", new Date());
		...
		session.setAttribute("dept", map);
		
		// 요청객체에 속성으로 저장된 데이터를 표현할 JSP로 내부이동 시키기
		RequestDispatcher rd = request.getRequestDispatcher("/WEB-INF/views/el/sample1.jsp");
