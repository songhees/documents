...
import kr.co.jhta.jstl.controller.JstlController5;
// 사용자정의 HttpServlet 클래스를 요청 URL과 매핑(a라는 URL 요청)
// http://localhost/model2-app/a
// hta로 끝나는 것은 모두 받는다.
@WebServlet("*.hta") 
public class FrontControllerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 웹 애플리케이션 ContextPath를 조회한다.
		 * ContextPath는 웹 애플리케이션을 식별하는 이름이다.
		 * 특별히 지정하지 않으면 프로젝트 이름이 contextPath가 된다.
		 * Tomcat에 배포된 여러 웹애플리케이션을 서로 다른 contextPath를 가지며, contextPath가 웹 애플리케이션 프로젝트를 구분하는 식별자가 된다.
		 */
		...
		// 요청 URI에 해당하는 컨트롤러 객체를 생성하고 실행시킨다.
		try {
			...
			// 어떤 URI로 CONTRoller과 연결시킬건지 설정
				...
				new JstlController5().execute(request, response);
			}
